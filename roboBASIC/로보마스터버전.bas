...
횟수_전진종종걸음_2B:
    ...
    보행COUNT = 보행COUNT + 1
    IF 보행COUNT > 보행횟수 THEN  GOTO 횟수_전진종종걸음_2_stopB

    ERX 4800,A, 횟수_전진종종걸음_4B
    IF A <> A_old THEN
횟수_전진종종걸음_2_stopB:
        MOVE G6D,95,  90, 125, 95, 104
        MOVE G6A,104,  76, 145,  91,  102
...
후진종종걸음_6:
    ...
    IF A <> A_old THEN  GOTO 후진종종걸음_6_stop
...
GOSUB All_motor_mode3
    ...
    SPEED 4
    MOVE G6A,100, 185,  28, 142, 100, 100
    MOVE G6D,100, 185,  28, 142, 100, 100
    MOVE G6B,160,  70,  80
    MOVE G6C,160,  70,  80
    WAIT
...
양팔벌리고왼쪽돌기:
	MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,100,  130,  80, 100, 100, 100
    MOVE G6C,100,  130,  80, 100, 100, 100
	Q=1 
	FOR Q=1 TO 2 '45도 회전 두번
    	MOTORMODE G6A,3,3,3,3,2
    	MOTORMODE G6D,3,3,3,3,2
    	SPEED 10
    	MOVE G6A,95,  106, 145,  63, 105, 100
...
양팔뒤로:
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,20,  30,  80, 100, 100, 100
    MOVE G6C,20,  30,  80, 100, 100, 100
    WAIT
    RETURN	
    '*************************************
양팔앞으로:
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,170,  30,  80, 100, 100, 100
    MOVE G6C,170,  30,  80, 100, 100, 100
    WAIT
    RETURN	
    '************************************************
거리인식걷기:
...
MAIN_2:
    ...
    ELSEIF A>=160 THEN
		A=A-160
		ON A GOTO KEY160,KEY161,KEY162,KEY163
		
	'계단 지역
	ELSEIF A>=131 THEN
		A=A-131 
		ON A GOTO KEY131,KEY132,KEY133,KEY134,KEY135,KEY136,KEY137,KEY138,KEY139
		
	'위험지역
	ELSEIF A>=122 THEN
		A=A-122 
		ON A GOTO KEY122,KEY123,KEY124,KEY125,KEY126,KEY127,KEY128,KEY129
		
	ELSEIF A>=110 THEN
		A=A-110
		ON A GOTO KEY110,KEY111,KEY112,KEY113,KEY114,KEY115,KEY116,KEY117,KEY118,KEY119,KEY120
